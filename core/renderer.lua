...
function M.update_status(window, pane)
  ...
  local tab_state_changed = (
    #tabs ~= status_cache.last_tab_count
    or (active_tab and active_tab:tab_id() ~= status_cache.last_active_tab)
  )

  if
    not tab_state_changed
    and status_cache.last_status
    and (current_time - status_cache.last_update) < min_update_interval
  then
    -- Use cached status
    local left_status, right_status =
      zones.split_status_for_display(status_cache.last_status, {
        window = window,
        pane = pane,
        tabs = tabs,
  ...
  local status_elements =
    zones.render_status_zones(left_components, right_components, context)
  ...
  local left_status, right_status =
    zones.split_status_for_display(status_elements, context)
  window:set_left_status(left_status)
  window:set_right_status(right_status)
end
...
function M.create_zone_components(component_names)
  ...
  for _, component_name in ipairs(component_names or {}) do
    ...
    local component =
      components.create_component(component_name, component_config)
...
function M.format_tab_title(tab, tabs, panes, wezterm_config, hover, max_width)
  ...
  local bg_color = tab.is_active and colors.active_tab.bg_color
    or colors.inactive_tab.bg_color
  local fg_color = tab.is_active and colors.active_tab.fg_color
